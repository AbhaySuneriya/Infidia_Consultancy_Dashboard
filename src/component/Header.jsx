...
function DropdownMenu({ children }) {
  ...
  return (
    <div className="relative inline-block">
      ...
      {isOpen && <div className="absolute mt-2 w-48 bg-white dark:bg-gray-800 shadow-lg rounded-md right-0">{children.slice(1)}</div>}
    </div>
  );
}
...
export default function Navbar() {
  ...
  return (
    <div className="flex w-full min-h-screen">
      {/* Sidebar */}
      <div className={`h-full bg-gray-200 dark:bg-gray-800 p-4 shadow-lg transition-all ${isSidebarOpen ? "w-64" : "w-0 overflow-hidden"}`}>
        {isSidebarOpen && (
          <>
            <Button variant="ghost" onClick={() => setIsSidebarOpen(false)} className="mb-4">
              Close
            </Button>
        ...
        <div className="flex items-center justify-between p-4 bg-gray-100 dark:bg-gray-900 shadow-md w-full">
          ...
          <div className="flex items-center gap-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuItem onClick={() => alert("Go to Dashboard")}>Dashboard</DropdownMenuItem>
